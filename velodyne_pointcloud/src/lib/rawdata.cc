...
void RawData::unpack(const velodyne_msgs::msg::VelodynePacket & pkt, DataContainerBase & data)
{
  ...
  RCLCPP_DEBUG_STREAM(
    node_ptr_->get_logger(), "Received packet, time: " << rclcpp::Time(pkt.stamp).seconds());
    ...
    for (int j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
      ...
      if (
        ...
        if (is_invalid_distance) {
          data.addPoint(
            x_coord, y_coord, z_coord, return_type, corrections.laser_ring, raw->blocks[i].rotation,
            0, intensity, time_stamp);
        } else {
          data.addPoint(
            x_coord, y_coord, z_coord, return_type, corrections.laser_ring, raw->blocks[i].rotation,
            distance, intensity, time_stamp);
        }
      }
    }
  }
  ...
  for (uint block = 0; block < BLOCKS_PER_PACKET; block++) {
    ...
    if (UPPER_BANK != raw->blocks[block].header) {
      ...
      RCLCPP_WARN_STREAM_THROTTLE(
        node_ptr_->get_logger(), *node_ptr_->get_clock(), 60000 /* ms */,
        "skipping invalid VLS-128 packet: block " << block << " header value is "
    ...
    if (
      (config_.min_angle < config_.max_angle && azimuth >= config_.min_angle &&
      ...
      for (int firing = 0, k = 0; firing < VLP16_FIRINGS_PER_BLOCK; ++firing) {
        for (int dsr = 0; dsr < VLP16_SCANS_PER_FIRING; dsr++, k += RAW_SCAN_SIZE) {
          ...
          }
          {
            ...
            // area (min_angle < area < max_angle).
            if (
              (azimuth_corrected >= config_.min_angle && azimuth_corrected <= config_.max_angle &&
               config_.min_angle < config_.max_angle) ||
              (config_.min_angle > config_.max_angle && (azimuth_corrected <= config_.max_angle ||
              ...
              if (is_invalid_distance) {
                data.addPoint(
                  x_coord, y_coord, z_coord, return_type, corrections.laser_ring, azimuth_corrected,
                  0, intensity, time_stamp);
              } else {
                data.addPoint(
                  x_coord, y_coord, z_coord, return_type, corrections.laser_ring, azimuth_corrected,
      ...
      for (uint j = 0, k = 0; j < SCANS_PER_BLOCK; j++, k += RAW_SCAN_SIZE) {
        ...
        }
        {
          ...
          if (
            ...
            const double time_stamp = block * 55.3 / 1000.0 / 1000.0 + j * 2.665 / 1000.0 / 1000.0 +
                                      rclcpp::Time(pkt.stamp).seconds();
