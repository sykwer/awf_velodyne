...
inline int get_concurrent_beams(uint8_t sensor_model)
{
  ...
  */
  ...
  switch (sensor_model) {
    ...
    default:
      RCLCPP_WARN_STREAM(
        rclcpp::get_logger("get_concurrent_beams"),
        "[Velodyne Ros driver]Default assumption of device id .. Defaulting to HDL64E with 2 "
 ...
 */
  if (packet_rmode == 57) {
    switch (sensor_model) {
      ...
      default:
        RCLCPP_WARN_STREAM(
          rclcpp::get_logger("get_rmode_multiplier"),
          "[Velodyne Ros driver]Default assumption of device id .. Defaulting to HDL64E with 2x "
          "number of packekts for Dual return");
        return (2);  // hdl-64e
    }
  } else {
    return (1);
...
: node_ptr_(node_ptr), diagnostics_(node_ptr_, 0.2)
{
  ...
  RCLCPP_INFO_STREAM(
    node_ptr_->get_logger(), deviceName << " rotating at " << config_.rpm << " RPM");
  ...
  RCLCPP_INFO_STREAM(
    ...
    "Scan start/end will be at a phase of " << config_.scan_phase << " degrees");
  ...
  output_ = node_ptr_->create_publisher<velodyne_msgs::msg::VelodyneScan>(
...
bool VelodyneDriverCore::poll(void)
{
  ...
  while (use_next_packet && rclcpp::ok()) {
    ...
    if (processed_packets > 1) {
      if (
        packet_last_azm_phased < packet_first_azm_phased ||
        packet_first_azm_phased < prev_packet_first_azm_phased) {
...
rcl_interfaces::msg::SetParametersResult VelodyneDriverCore::paramCallback(
