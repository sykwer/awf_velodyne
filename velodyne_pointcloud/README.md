...
* 回転情報のハッシュ群
    ...
    `Interpolate`ノードのメンバ`twist_queue_`（`std::deque<geometry_msgs::msg::TwistStamped>`型）が持つ情報を格納したハッシュ群。以下のフィールドを持つ。

    * `stamp`要素配列のハッシュ

        キー：`stamp`、値：`(header.stamp要素)`を持つハッシュ。ただし、時間は`RCL_SYSTEM_TIME`で表され、単位は`ns`とする。

    * `linear`要素のハッシュ

        キー：`linear`、値：`(twist.linear要素)`を持つハッシュ。

    * `angular`要素のハッシュ

        キー：`angular`、値：`(angular要素)`を持つハッシュ。
...
- frame_id: 0
  cloud:
    points:
      - [0.9598088, 1.769561, -0.01282597, 27, 1, 0, 58, 2.1, 1585897258.110364]
      ...
      - [7.348701, -6.598307, 11.98354, 1, 1, 14, 15139, 14.082, 1585897258.152284]
    width: 5771
    height: 1
    is_dense: true
  twist:
    - stamp: 1585897266292893184
      linear: [0, 0, 0]
      angular: [0, 0, -0]
      ...
    - stamp: 1585897267264281600
      linear: [0, 0, 0]
      angular: [0, 0, -0]
...
- frame_id: 9
  cloud:
    points:
      - [-22.95948, -0.06209081, 6.591853, 2, 1, 11, 26044, 24.376, 1585897281.282459]
      ...
      - [1.008074, 4.119726, 0.01999664, 4, 1, 15, 36, 3.922, 1585897281.410331]
    width: 19237
    height: 1
    is_dense: true
  twist:
...
出力テストベクタのフォーマットは、入力テストベクタのフォーマットのうち、フレームの配列内から回転のハッシュを除いたものである。
