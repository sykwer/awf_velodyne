...
{
static const size_t packet_size = sizeof(velodyne_msgs::msg::VelodynePacket().data);
 ...
 */
Input::Input(rclcpp::Node * node_ptr, uint16_t port) : node_ptr_(node_ptr), port_(port)
{
  ...
  if (!devip_str_.empty())
    RCLCPP_INFO_STREAM(
      node_ptr_->get_logger(), "Only accepting packets from IP address: " << devip_str_);
}
 ...
 *  @param port UDP port number
 */
InputSocket::InputSocket(rclcpp::Node * node_ptr, uint16_t port) : Input(node_ptr, port)
{
  sockfd_ = -1;
...
int InputSocket::getPacket(velodyne_msgs::msg::VelodynePacket * pkt, const double time_offset)
{
  ...
  while (rclcpp::ok()) {
    ...
    RCLCPP_DEBUG_STREAM(
      node_ptr_->get_logger(), "incomplete Velodyne packet read: " << nbytes << " bytes");
  }
 ...
 */
InputPCAP::InputPCAP(
  rclcpp::Node * node_ptr, uint16_t port, std::string filename, bool read_once, bool read_fast,
  double repeat_delay)
: Input(node_ptr, port),
  ...
  filename_(filename)
{
  ...
  if (read_once_) RCLCPP_INFO(node_ptr_->get_logger(), "Read input file only once.");
  if (read_fast_) RCLCPP_INFO(node_ptr_->get_logger(), "Read input file as quickly as possible.");
  if (repeat_delay_ > 0.0)
    RCLCPP_INFO(
      node_ptr_->get_logger(), "Delay %.3f seconds before repeating input file.", repeat_delay_);
...
int InputPCAP::getPacket(velodyne_msgs::msg::VelodynePacket * pkt, const double time_offset)
{
  ...
  while (true) {
    ...
    if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0) {
      ...
      // selected IP address.
      if (/* !devip_str_.empty() &&  Let the filter take care of skipping bad packets ... not
             dependent on device IP setting*/
      ...
      pkt->stamp = rosTimeFromGpsTimestamp(
        t, &(pkt->data[TIMESTAMP_BYTE]));  // time_offset not considered here, as no synchronization
                                           // required
      empty_ = false;
    ...
    {
      RCLCPP_WARN(
        node_ptr_->get_logger(), "Error %d reading Velodyne packet: %s", res, pcap_geterr(pcap_));
      return -1;
    }
    ...
    if (repeat_delay_ > 0.0) {
      RCLCPP_INFO(
        node_ptr_->get_logger(), "end of file reached -- delaying %.3f seconds.", repeat_delay_);
